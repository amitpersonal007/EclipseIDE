...
@Action(actionText = "Enter testdata inside an element using keypad", applicationType = ApplicationType.ANDROID)
   ...
   try{
     ...
     for(char c:arr){
        ...
        if(Character.isLowerCase(c)){
     	   ...
     	   d.pressKey(new KeyEvent(AndroidKey.MINUS).withMetaModifier(KeyEventMetaModifier.SHIFT_ON)); 
        }
        else if(String.valueOf(c).equals("-")){
        	...
        	d.pressKey(new KeyEvent(AndroidKey.DIGIT_4).withMetaModifier(KeyEventMetaModifier.SHIFT_ON));
        }
